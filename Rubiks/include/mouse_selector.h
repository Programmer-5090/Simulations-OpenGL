...
public:
    ...
    /**
     * @brief Shoots a ray from the camera center and selects the closest hit.
     * @return ID of closest intersected object, or nullopt if none
     */
    std::optional<ID> selectFromCameraCenter();
    
    /**
     * @brief Shoots rays from camera in a given direction.
    ...
    /**
     * @brief Shoots rays from a custom origin in a given direction.
    ...
    /**
     * @brief Shoots a grid of rays and returns all unique objects hit.
     * @param gridSize Number of rays per dimension (gridSize × gridSize)
     * @param spreadAngle Angular spread of the grid in degrees
     * @param cam Camera to use for ray generation
     * @return Vector of unique IDs that were hit by any ray
     */
