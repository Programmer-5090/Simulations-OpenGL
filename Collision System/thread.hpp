...
struct Threader {
    ...
    Threader(int num_threads_ = (int)std::thread::hardware_concurrency())
        : num_threads{num_threads_ ? num_threads_ : 1}
    {
    ...
    void parallel(int num_obj, std::function<void(int start, int end)>&& callback) {
        ...
        if (slice_size == 0) {
            // fewer objects than threads; just run single task
            callback(0, num_obj);
            return;
        }
        for (int i = 0; i < num_threads; i++) {
            int start = i * slice_size;
            int end = (i == num_threads - 1) ? std::min(num_obj, start + slice_size) : start + slice_size;
            t_queue.addTask([start, end, &callback](){ callback(start, end);});
        }
        int covered = slice_size * num_threads;
        if (covered < num_obj) {
            int start = covered;
